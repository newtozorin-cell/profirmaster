...
# ========================================
FYERS_APP_ID     = os.environ.get('FYERS_API_KEY', '')
FYERS_SECRET_KEY = os.environ.get('FYERS_API_SECRET', '')
...
def direction_allowed(symbol, dt, direction):
    """direction is 'BUY-LONG' or 'SELL-SHORT'."""
...
def notify_new_signals(new_signals):
    if not new_signals:
        return
    # Market hours guard: 09:00-23:59 IST on trading days
    # (covers GOLDPETAL's 9am-midnight MCX session)
...
def generate_signals():
    ...
    for symbol, config in SCANNER_CONFIG.items():
        try:
            ...
            # Opening 1-min scan: catches quick-entry crosses in 09:15-09:20
            ...
            for _, row in scan_df.iterrows():
                ...
                # Skip the 09:15 5-min bar if a 1-min opening signal already fired in 09:15-09:20
...
def home():
    ...
    <div id="signals-box" style="background:#1a2a4a;padding:20px;margin:20px auto;max-width:900px;border-radius:8px;text-align:left;">
        <h3 style="margin-top:0;color:#22d3ee">📊 Recent Signals <span id="src-tag" style="font-size:11px;color:#888;margin-left:10px"></span></h3>
        <div id="signals-list">Loading…</div>
    </div>
    <script>
    // Browser-side cache: keeps last good signals even if /api/signals briefly returns empty
    const CACHE_KEY = 'striketrail_signals_v1';
    function renderSignals(signals, source) {{
        const list = document.getElementById('signals-list');
        const tag = document.getElementById('src-tag');
        tag.textContent = source === 'cache' ? '(from browser cache)' : source === 'api' ? '(live)' : '';
        if (!signals || signals.length === 0) {{
            list.innerHTML = '<p style="color:#888">No signals yet.</p>';
            return;
        }}
        let html = '<table style="width:100%;border-collapse:collapse;font-size:13px"><tr style="background:#0f1f3d">'
            + '<th style="padding:6px;text-align:left">Time</th>'
            + '<th>Symbol</th><th>Dir</th><th>Entry</th><th>SL</th><th>T1</th><th>T2</th></tr>';
        for (const s of signals.slice(0, 30)) {{
            html += '<tr style="border-bottom:1px solid #333">'
                + '<td style="padding:6px">' + (s.scan_time || '') + '</td>'
                + '<td>' + (s.symbol || '') + '</td>'
                + '<td style="color:' + (s.direction === 'BUY-LONG' ? '#22c55e' : '#ef4444') + '">' + (s.direction || '') + '</td>'
                + '<td>' + (s.entry || '') + '</td>'
                + '<td>' + (s.sl || '') + '</td>'
                + '<td>' + (s.target_1 || '') + '</td>'
                + '<td>' + (s.target_2 || '') + '</td></tr>';
    ...
    async function fetchSignals() {{
        try {{
            const r = await fetch('/api/signals?force=true', {{ cache: 'no-store' }});
            const data = await r.json();
            if (data.signals && data.signals.length > 0) {{
                localStorage.setItem(CACHE_KEY, JSON.stringify({{ ts: Date.now(), signals: data.signals }}));
                renderSignals(data.signals, 'api');
            }} else {{
                // API returned empty — fall back to browser cache
                const cached = JSON.parse(localStorage.getItem(CACHE_KEY) || 'null');
                if (cached && cached.signals && cached.signals.length > 0) {{
                    renderSignals(cached.signals, 'cache');
                }} else {{
                    renderSignals([], '');
                }}
            }}
            ...
            const cached = JSON.parse(localStorage.getItem(CACHE_KEY) || 'null');
            renderSignals(cached ? cached.signals : [], 'cache');
        }}
    }}
    // First load: show cache immediately, then refresh from API
    (function() {{
        const cached = JSON.parse(localStorage.getItem(CACHE_KEY) || 'null');
        if (cached) renderSignals(cached.signals, 'cache');
    }})();
    fetchSignals();
    // Refresh every 30s
...
def api_signals():
    ...
    if scan_lock.acquire(blocking=False):
        try:
            ...
            # Only overwrite cache if we got something
            if signals:
                scan_cache['signals'] = signals
