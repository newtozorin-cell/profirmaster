...
def auto_refresh_access_token():
    ...
    try:
        ...
        print(f"✗ Refresh rejected by Fyers: {r.text[:300]}")
        return False
    except Exception as e:
...
def scan_opening_signal_1min(symbol, config, df_1m):
    ...
    for _, row in opening.iterrows():
        ...
        # 09:15 cross -> entry = 09:16 candle close; 09:16+ cross -> same candle close
        row_hhmm = row['datetime'].hour * 100 + row['datetime'].minute
        if row_hhmm == 915:
            next_row = df[df['datetime'] == row['datetime'] + timedelta(minutes=1)]
            if len(next_row) == 0:
                continue  # 09:16 candle not yet available
            entry = round(float(next_row.iloc[0]['close']), 2)
        else:
